...
from lib.auth import add_user, delete_user, list_users, user_exists, is_admin_user
...
def user_manager():
    ...
    while True:
        console.print(Panel(
            ...
            "[cyan]3.[/cyan] Delete user\n"
            "[cyan]4.[/cyan] Return\n",
            title="[bold]User Management[/bold]",
            border_style="blue",
            box=box.ROUNDED
        ))
        choice = Prompt.ask("[cyan]Select an option[/cyan]", default="4")
        if choice == "1":
            rows = list_users()
            if not rows:
                ...
                for row in rows:
                    created = time.strftime('%Y-%m-%d', time.gmtime(row[4]))
                    table.add_row(str(row[0]), row[1], "✓" if row[2] else "", row[3] or "-", created)
                console.print(table)
        elif choice == "2":
